...
bool play_sound(const char* filename) {
    ...
    play_sample(file, 0 /* card */, 13 /* device */, chunk_fmt.num_channels, chunk_fmt.sample_rate,
...
int main() {
    if (const auto mixer = mixer_open(0)) {
        ...
        play_sound("/vendor/etc/audio/silence_48k.wav");

        // Disable speaker
        set_mixer_value_by_name(mixer, "QUAT_MI2S_RX Audio Mixer MultiMedia5", 0);
